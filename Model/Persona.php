	...
	class Persona {
	    ...
	    public function usuario() {
	   	...
	   	public function audiencias() {
	    	$con = new Connection();
	    	$con = $con->getConnection();
	    	$sql = "SELECT audiencia.id as id, audiencia.status as status, audiencia.fecha as fecha, hora, inconformidad_id, conciliador_id FROM audiencia join inconformidad on inconformidad.id = inconformidad_id join persona on persona.id = consumidor_id where persona.id = $this->id";
	    	$rs = $con->query($sql);
	    	$audiencias = [];
			if ($rs->num_rows > 0)
				while($row = $rs->fetch_assoc()) {
